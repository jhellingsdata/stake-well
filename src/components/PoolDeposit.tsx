...
export function PoolDeposit() {
    ...
    const { ethBalance = 'Loading...', ethSymbol = 'ETH', refetchEthBalance } = useAccountBalance();
    const { stEthBalance = 'Loading...', stEthSymbol = 'stETH', refetchStEthBalance } = useAccountTokenBalance();
                ...
                <Connected>
                    <div className="flex w-full justify-between items-center"> {/* This will center align the contents */}
                        ...
                        <div className="text-sm">
                            ...
                            <div>
                                {parseFloat(ethBalance).toFixed(decimals) ?? 'Loading...'} {ethSymbol ?? 'ETH'}
                            </div>
                            <div>
                                {parseFloat(stEthBalance).toFixed(decimals) ?? 'Loading...'} {stEthSymbol ?? 'stETH'}
                            </div>
                        </div>
                    </div>
