...
export const PoolUserBalance: React.FC<PoolUserBalanceProps> = ({
    decimals = 18,
}) => {
    return (
        <div>
    ...
    return (
        <div>
            <div className="raffle-card w-[400px] flex-initial flex-row mt-10 mx-auto px-6">
                <CustomButton
                    title="Deposit"
                    handleClick={() => setOpenDeposit(!openDeposit)}
                    containerStyles={buttonStyles}
                />
                <CustomButton
                    title="Withdraw"
                    handleClick={() => setOpenWithdraw(!openWithdraw)}
                    containerStyles={buttonStyles}
                />
                stETH balance:{' '}
                {isLoading
