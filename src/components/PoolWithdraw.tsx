...
import {
    useRafflePoolGetUserDeposit,
    useRafflePoolWithdrawStEth,
    usePrepareRafflePoolWithdrawStEth,
} from '../generated';
import { useDebounce } from '../hooks/useDebounce';

export function PoolWithdraw() {
    const { address } = useAccount();
    const { data: userWalletBalance, isLoading: isUserWalletBalanceLoading } =
        useBalance({
    ...
    const { write, data, error, isLoading, isError } =
        useRafflePoolWithdrawStEth(config);
                ...
                <Connected>
                    <div className="flex w-full justify-between items-center">
                        ...
                        <div className="text-sm">
                            ...
                            <div>
                                {balance ? formatEther(balance) : `error`}
                            </div>
                        </div>
                    </div>
